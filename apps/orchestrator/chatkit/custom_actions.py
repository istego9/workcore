...
def normalize_custom_action_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
...
def _normalize_mapping(value: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, item in value.items():
        if key in _PAYLOAD_WRAPPER_KEYS and isinstance(item, dict):
            ...
            continue
        if key == "documents":
            # Pass document payload through unchanged; binary/content trimming is handled by run projections.
            normalized[key] = item
            continue
